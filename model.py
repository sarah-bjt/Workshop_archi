...
def temps_par_exo(user_id) :
  mycursor.execute("SELECT exo_nom, crs_nom, rps_tps_passe, EXO.exo_id FROM t_cours_crs AS CRS JOIN t_exercice_exo AS EXO ON CRS.crs_id=EXO.crs_id JOIN t_reponse_rps AS RPS ON EXO.exo_id = RPS.exo_id JOIN t_compte_cpt AS CPT ON RPS.cpt_identifiant = CPT.cpt_identifiant WHERE CPT.cpt_identifiant=%(id)s ORDER BY exo_nom ASC;", {'id': user_id})
  return mycursor.fetchall()
...
def creation_compte_profil(email, password, name, surname):
    ...
    mydb.commit()
    mycursor.execute("INSERT INTO t_profil_pfl (pfl_nom, pfl_prenom, pfl_statut, pfl_date_creation, cpt_identifiant) VALUES (%(na)s, %(su)s, 'ETUDIANT', CURDATE(), %(em)s)", {'na': name, 'su': surname, 'em': email})
    mydb.commit()
